...
public class SectorProductivoPruebaFrame {
    private DefaultTableModel tableModel;
    private JTable table;
    private List<SectorProductivo> sectorList;
    private JPanel mainPanel;

    public SectorProductivoPruebaFrame() {
        ...
        addValidBtn.addActionListener(e -> addSector(true));
        addInvalidBtn.addActionListener(e -> addSector(false));
    ...
    private void addSector(boolean valido) {
        JTextField nombre = new JTextField();
        JTextField hectareas = new JTextField();
        Object[] fields = {"Nombre:", nombre, "Hectáreas:", hectareas};
        int res = JOptionPane.showConfirmDialog(mainPanel, fields, (valido ? "Agregar Sector Válido" : "Agregar Sector Inválido"), JOptionPane.OK_CANCEL_OPTION);
        if (res == JOptionPane.OK_OPTION) {
            ...
            try {
                ...
                refreshTable();
                JOptionPane.showMessageDialog(mainPanel, "Sector productivo agregado correctamente.");
    ...
    private void exportarResultados() {
        try {
            java.io.PrintWriter pw = new java.io.PrintWriter("sectorproductivo_prueba_resultados.csv");
            pw.println("Nombre,Hectáreas");
            for (SectorProductivo s : sectorList) {
                pw.println(s.getNombre() + "," + s.getExtended());
            }
            pw.close();
            JOptionPane.showMessageDialog(mainPanel, "Resultados exportados correctamente a sectorproductivo_prueba_resultados.csv");
        } catch (Exception ex) {
